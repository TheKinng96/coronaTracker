...
async function initMap() {
    ...
    let malaysia = { lat: 4.2105, lng: 101.9758 }
...
function displayInfectedCountries(countries) {
    ...
    countries.map((i, index) => {
        storesHtml += `
            <div id=${i['countryInfo']['_id']} class="store-container" onclick="clickStore('${i['countryInfo']['iso2']}')">
                ...
                <div class="store-number-container">
                    ...
                    <img src="${i['countryInfo']['flag']}" width="50px" height="30px"/>
                    </div>
                </div>
            </div>
        `
...
function showMarkers(search) {
    search.map((i, index) => {
        ...
        createMarker(latlng, lastUpdated, country, cases, deaths, recovered, id, iso2)
    });
}

function createMarker(latlng, lastUpdated, country, cases, deaths, recovered, id, iso2) {
    let html = `
...
function clickStore(index) {
    ...
    let selectedMarker = markers.find(i => i.label.toLowerCase() === index.toLowerCase())
